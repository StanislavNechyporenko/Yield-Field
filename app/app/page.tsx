...
export default function Home() {
  ...
  // Deep link from the landing: /app?p=<pool id> preselects the pool
  // (switching the asset tab if needed) and scrolls to its details.
  const [deepLinkApplied, setDeepLinkApplied] = useState(false);
  useEffect(() => {
    if (deepLinkApplied || protocols.length === 0) return;
    setDeepLinkApplied(true);
    const target = protocols.find(
      (p) => p.id === new URLSearchParams(window.location.search).get('p')
    );
    if (!target) return;
    setSelectedAsset((target.asset ?? 'MON') as InvestAsset);
    setSelectedProtocol(target.id);
    setTimeout(() => {
      document.getElementById('protocol-details')?.scrollIntoView({ behavior: 'smooth' });
    }, 150);
  }, [protocols, deepLinkApplied]);
